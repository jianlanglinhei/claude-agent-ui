...
export default function DirectoryPanel({ agentDir }: DirectoryPanelProps) {
  ...
  return (
    <div
      className={`flex h-full flex-col border-l border-[var(--line)] bg-[var(--paper-contrast)]/70 transition-all duration-300 ${isCollapsed ? 'w-12' : 'w-80 lg:w-96'}`}
    >
      {isCollapsed ?
        <div className="flex h-full flex-col items-center">
          <div className="flex flex-1 items-center justify-center py-4">
            <button
              type="button"
              onClick={() => {
                console.log('Expanding panel from vertical arrow');
                setIsCollapsed(false);
              }}
              className="action-button flex items-center justify-center p-2 transition-colors hover:bg-[var(--paper-strong)]"
              title="Show directory panel"
            >
              <ChevronLeft className="h-5 w-5 text-[var(--ink-muted)]" />
            </button>
          </div>
        </div>
      : <>
          <div className="border-b border-[var(--line)] px-4 py-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="action-button p-1 hover:bg-[var(--paper-strong)]"
                title="Hide directory panel"
              >
            ...
            {directoryInfo && (
              <div className="mt-2 text-[11px] text-[var(--ink-muted)]">
                Files {directoryInfo.summary.totalFiles} - Directories{' '}
                {directoryInfo.summary.totalDirs}
          ...
          <div className="relative z-10 border-b border-[var(--line)] px-4 py-3 text-[11px] text-[var(--ink-muted)]">
            <div className="flex items-center gap-2">
              ...
              {isUploading && (
                <span className="text-[11px] text-[var(--ink-muted)]">Uploading...</span>
              )}
            </div>
          </div>
